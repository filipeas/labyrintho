...
def remap_color_gt(img):
    ...
    gt_color_array = label_cmap(img_normalized)  # Mapeia os valores normalizados para as cores do colormap

    # Converte para formato de imagem RGB e ajusta para o intervalo [0, 255]
    gt_color_img = (gt_color_array[:, :, :3] * 255).astype(np.uint8)  # Retorna para o intervalo [0, 255] e converte para uint8
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Processa uma imagem, converte para escala de cinza e aplica uma máscara.")
    ...
    parser.add_argument("only_iou", type=str, help="Quer processar somente o nivel de pixel do iou ou toda a label.")
