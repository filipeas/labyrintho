import torch
import numpy as np
from minerva.transforms.transform import Transpose, PadCrop, CastTo
...
class Segmenter:
    def __init__(
        self, multimask_output: bool, height: float, width: float, model, device
    ):
    ...
    def segment(self, image: np.ndarray, points: list):
        ...
        input_points = (
            torch.tensor([[x, y] for x, y, _ in points], dtype=torch.long)
            .unsqueeze(0)
            .to(self.device)
        )  # shape: (N, 2)
        input_labels = (
            torch.tensor([label for _, _, label in points], dtype=torch.long)
            .unsqueeze(0)
            .to(self.device)
        )  # shape: (N,)
        ...
        )  # [B, 3, H, W] if multimask_output else [B, 1, H, W]
